...
public class UserService {
	...
	public static Admin adLoginCheck(Admin admin){
		...
		String password = admin.getAd_password();
		if(mes != null){
	...
	public static List<User> queryUser(String username) {
		List<User> list = u.qUser(username);
		return list;
	}
	
	//返回根据姓名查询到的user数组
	public static List<User> queryByName(String name) {
		List<User> list = u.qByName(name);
		return list;
	}	
	
	public static List<User> queryByTel(String tel) {
		List<User> list = u.qByTel(tel);
		return list;
	}
	...
	public static boolean adRegister(Admin admin) {
		...
		admin.setAd_password(admin.getAd_password());
